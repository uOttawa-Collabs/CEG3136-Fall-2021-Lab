...
polldelay:
    ...
    ldx #3000              ; Set loop counter X to 3000
polldelayLoop:
    nop                    ; 4 NOPs
    nop
    nop
    nop
    dex                    ; Decrement X
    bne polldelayLoop      ; Continue loop if X is not 0
    
    ldx delayCount         ; Decrement delayCount
    dex                    ; Note that since DEC only decrements 8-bit number in memory,
    stx delayCount         ; we have to load delayCount into X to do 16-bit decrement.
    beq polldelayFinished  ; If X (delayCount) is 0, goto polldelayFinished;
    ldaa #FALSE            ; else return false in accumulator A.
    bra polldelayExit
polldelayFinished:
    ldaa #TRUE             ; Return true in accumulator A.
polldelayExit:
